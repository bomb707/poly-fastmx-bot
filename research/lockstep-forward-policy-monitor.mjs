#!/usr/bin/env node
/**
 * Generic frozen forward-paper monitor for a Lockstep policy.
 *
 * This module imports no order client and cannot submit CLOB orders. Each PM2
 * instance must provide a policy, manifest, output directory, state file, and
 * predeclared cohort start through environment variables.
 */
...
const resolveInput = (name) => path.resolve(ROOT, required(name));
const START_MS = Date.parse(required("LOCKSTEP_FORWARD_START"));
...
const POLICY = resolveInput("LOCKSTEP_FORWARD_POLICY");
const MANIFEST = resolveInput("LOCKSTEP_FORWARD_MANIFEST");
const DATA = resolveInput("LOCKSTEP_FORWARD_DATA_DIR");
const OUTPUT = path.join(DATA, "latest");
const STATE = resolveInput("LOCKSTEP_FORWARD_STATE");
