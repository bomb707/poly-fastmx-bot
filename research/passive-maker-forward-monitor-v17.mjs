#!/usr/bin/env node
/** Forward-only dual-orderbook monitor for the frozen v17 residual candidate. No order client is imported. */
...
const START_MS = Date.parse(process.env.MAKER_FORWARD_START || "2026-08-25T12:55:00Z");
const LAG_MS = Number(process.env.MAKER_FORWARD_RESOLUTION_LAG_MS || 180_000);
const POLL_MS = Math.max(300_000, Number(process.env.MAKER_FORWARD_POLL_MS || 3_600_000));
...
const DATA = path.resolve(process.env.MAKER_FORWARD_DATA_DIR || path.join(ROOT, "data/passive-maker-forward-v17"));
const V4_L2 = path.join(DATA, "feeds/v4-l2"), V2 = path.join(DATA, "feeds/v2");
const V2_L2 = path.join(DATA, "feeds/v2-l2"), TRADES = path.join(DATA, "feeds/market-trades");
const BATCHES = path.join(DATA, "batches"), CHECKPOINT_V2 = path.join(DATA, "checkpoint-v2.json");
const CHECKPOINT_V4 = path.join(DATA, "checkpoint-v4.json");
const STATE = path.resolve(process.env.MAKER_FORWARD_STATE || path.join(ROOT, "data/research/passive-maker-forward-v17-state.json"));
const SELECTED = path.resolve(process.env.MAKER_FORWARD_SELECTED || path.join(ROOT, "research/passive-maker-v17-residual-selected.json"));
const MANIFEST = path.resolve(process.env.MAKER_FORWARD_MANIFEST || path.join(ROOT, "research/passive-maker-forward-v17-manifest.json"));
const REPLAY = path.join(ROOT, "research/passive-maker-walkforward-v17-residual.mjs");
...
const child = async (args, env = {}) => {
  const result = await runFile(process.execPath, args, { cwd: ROOT, env: { ...process.env, ...env }, maxBuffer: 32 * 1024 * 1024 });
...
function sourceAssessment(output, source, targetMs) {
  ...
  const loaded = Number(output.range?.loaded || 0), failed = Number(output.range?.failed || 0);
  ...
  const requirements = {
    ...
    postOnlyInvariant: rows.every((row) => row.params?.postOnly === true),
    ...
    noAutomaticTakerHedge: rows.every((row) => row.params?.safeHedgeEveryTick === false
      && Number(row.params?.endLiquidateS) === 0 && Number(row.params?.unpairedTimeoutS) === 0),
    fiveShareResidualTarget: rows.every((row) => Number(row.params?.residualTargetShares) === 5
      && Number(row.params?.residualBaseOrderShares) === 5),
  };
  return { source, coverage: { expected, discovered: output.range?.discovered || 0, loaded, failed,
    pct: expected ? loaded / expected * 100 : 0 }, focus: { activeWindows: focus.activeWindows || 0,
    pnl: focus.pnl || 0, profitFactor: focus.profitFactor, maxDrawdown: focus.maxDrawdown || 0,
    bootstrapWindowLower95: focus.bootstrapWindowLower95, bootstrapDayLower95: focus.bootstrapDayLower95,
    pairedPnl: focus.pairedPnl || 0, residualPnl: focus.residualPnl || 0 }, folds, requirements,
...
async function cycle() {
  ...
  const replayEnv = { MAKER_V2_DIRS: V2, MAKER_TRADE_DIRS: TRADES, MAKER_FILL_SOURCE: "trades",
    MAKER_TRADE_PRICE_MODE: "exact", MAKER_POLICIES_FILE: SELECTED, MAKER_LATENCIES: "130,200,300",
    MAKER_CREDITS: ".025,.05,.075,.1", MAKER_TAKER_LATENCY_MS: "520", MAKER_BOOTSTRAP_SAMPLES: "20000",
    MAKER_QUIET: "1" };
  await child([REPLAY, iso(START_MS), iso(target), CHECKPOINT_V2], { ...replayEnv, MAKER_L2_DIRS: V2_L2, MAKER_CONFIRM_L2_DIRS: V4_L2 });
  await child([REPLAY, iso(START_MS), iso(target), CHECKPOINT_V4], { ...replayEnv, MAKER_L2_DIRS: V4_L2, MAKER_CONFIRM_L2_DIRS: V2_L2 });
  ...
  const assessment = { assessedAt: new Date().toISOString(), cohortStart: iso(START_MS), target: iso(target), elapsedDays,
    mode: "research-only-no-orders", manifest, sources: { v2, v4 }, requirements: {
      frozenManifestMatches: manifest.passed, elapsedDurationDays30: elapsedDays >= 30,
      nativeV2Passes: v2.passed, v4Passes: v4.passed } };
  assessment.passed = Object.values(assessment.requirements).every(Boolean);
  fs.writeFileSync(STATE, JSON.stringify({ schema: 1, assessment }, null, 2) + "\n");
  lastTarget = target;
