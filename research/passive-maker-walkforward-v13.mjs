...
function normalize(meta) {
  ...
  const ticks = (raw.ticks || []).map((tick) => {
    ...
    if (!Array.isArray(tick.up) || !Array.isArray(tick.down)) return { ms, cl, up: tick.up, down: tick.down };
    const rows = (flat) => {
      const out = [];
  ...
  return ticks.length > 20
    ? { slug: meta.slug, startMs: slugStart(meta.slug), openChainlink, winner, ticks, trades }
...
function replayWindow(feed, p) {
  ...
  for (const tick of feed.ticks) {
    ...
    if (!p.makerTradingEnabled || t < p.effectiveMinTimeS || t >= p.maxTimeS || timedRiskExit
  ...
  return { slug: feed.slug, startMs: feed.startMs, winner: feed.winner, placements, cancels, rejected, firstPlacementT, up, down, makerShares, takerShares, takerBuyShares, takerSellShares, makerFillEvents, takerFillEvents,
