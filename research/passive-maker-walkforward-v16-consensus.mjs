...
function replayWindow(feed, p) {
  ...
  for (const tick of feed.ticks) {
    ...
    for (let index = arriving.length - 1; index >= 0; index--) {
      ...
      if (order.shares > 1e-9) {
        ...
        const normalExpiryMs = tick.ms + p.ttlMs + p.cancelLatencyMs;
    ...
    let wanted = p.signalMode === "twoSided"
    ...
    if (balancedOpening && p.confirmPairQuoteCap > 0) {
  ...
  return { slug: feed.slug, startMs: feed.startMs, winner: feed.winner, placements, cancels, rejected, confirmationUnavailable, confirmationGateRejected, overweightCancelTriggers, overweightCancelRequests, firstTimedExitT, firstTimedExitSide, firstTimedExitClAlignedPct, firstTimedExitBzAlignedPct, firstPlacementT, up, down, makerShares, takerShares, takerBuyShares, takerSellShares, makerFillEvents, takerFillEvents,
