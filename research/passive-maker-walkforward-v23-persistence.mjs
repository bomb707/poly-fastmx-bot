...
const SLUG_ALLOWLIST_FILE = String(process.env.MAKER_SLUG_ALLOWLIST_FILE || "").trim();
const SLUG_ALLOWLIST_PAYLOAD = SLUG_ALLOWLIST_FILE
  ? JSON.parse(fs.readFileSync(path.resolve(SLUG_ALLOWLIST_FILE), "utf8")) : null;
const SLUG_METADATA = new Map((Array.isArray(SLUG_ALLOWLIST_PAYLOAD?.markets)
  ? SLUG_ALLOWLIST_PAYLOAD.markets : []).map((market) => [String(market?.slug || ""), market]));
const SLUG_ALLOWLIST = (() => {
  if (!SLUG_ALLOWLIST_FILE) return null;
  const parsed = SLUG_ALLOWLIST_PAYLOAD;
  const values = Array.isArray(parsed) ? parsed
    : Array.isArray(parsed?.slugs) ? parsed.slugs
      : Array.isArray(parsed?.markets) ? parsed.markets
      : Object.values(parsed?.diagnostics || {}).find((entry) => Array.isArray(entry?.windowsDetail))?.windowsDetail;
  if (!Array.isArray(values)) throw new Error("MAKER_SLUG_ALLOWLIST_FILE must contain an array, {slugs}, or diagnostics with windowsDetail");
  const slugs = new Set(values.map((value) => String(value?.slug || value)).filter(Boolean));
  if (!slugs.size) throw new Error("MAKER_SLUG_ALLOWLIST_FILE contains no slugs");
  return slugs;
})();
...
const INCLUDE_SPARSE_WINDOWS = process.env.MAKER_INCLUDE_SPARSE_WINDOWS === "1";
const INCLUDE_UNAVAILABLE_WINDOWS = process.env.MAKER_INCLUDE_UNAVAILABLE_WINDOWS === "1";
const EXACT_FLOW_DIR = path.resolve(process.env.MAKER_EXACT_FLOW_DIR
  || path.join(ROOT, "data/research/strict-maker-flow-transactions"));
const STRICT_CANDIDATE_OUTPUT = path.resolve(process.env.MAKER_STRICT_CANDIDATE_OUTPUT
  || path.join(ROOT, "data/research/strict-maker-flow-candidates.json"));
const strictCandidateTransactions = new Map();
const discoveryCoverage = { expected: 0, complete: 0, missing: [], confirmationMissing: [] };
const normalizationFailureRows = new Map();
let currentNormalizationSlug = null;
...
const rejectNormalization = (reason) => {
  ...
  if (currentNormalizationSlug) normalizationFailureRows.set(currentNormalizationSlug, reason);
  return null;
};
...
function discover() {
  const l2 = new Map(), l2Candidates = new Map(), confirmL2 = new Map(), confirmL2Candidates = new Map();
  const v2 = new Map(), trades = new Map();
  for (const dir of L2_DIRS) {
    if (!fs.existsSync(dir)) continue;
    for (const name of fs.readdirSync(dir)) {
      if (!name.endsWith(".json.gz")) continue;
      const slug = name.slice(0, -8), startMs = slugStart(slug);
      if (startMs >= FROM_MS && startMs < TO_MS && (!SLUG_ALLOWLIST || SLUG_ALLOWLIST.has(slug))) {
        const file = path.join(dir, name);
        if (!l2.has(slug)) l2.set(slug, file);
        if (!l2Candidates.has(slug)) l2Candidates.set(slug, []);
        l2Candidates.get(slug).push(file);
      }
    }
  }
  for (const dir of CONFIRM_L2_DIRS) {
    ...
    for (const name of fs.readdirSync(dir)) {
      if (!name.endsWith(".json.gz")) continue;
      const slug = name.slice(0, -8), startMs = slugStart(slug);
      if (startMs >= FROM_MS && startMs < TO_MS) {
        const file = path.join(dir, name);
        if (!confirmL2.has(slug)) confirmL2.set(slug, file);
        if (!confirmL2Candidates.has(slug)) confirmL2Candidates.set(slug, []);
        confirmL2Candidates.get(slug).push(file);
      }
    }
  }
  for (const dir of V2_DIRS) {
    ...
    for (const name of fs.readdirSync(dir)) {
      ...
      if (startMs >= FROM_MS && startMs < TO_MS && (!SLUG_ALLOWLIST || SLUG_ALLOWLIST.has(slug)) && !v2.has(slug))
        v2.set(slug, path.join(dir, name));
    }
  }
  for (const dir of TRADE_DIRS) {
    ...
    for (const name of fs.readdirSync(dir)) {
      ...
      const slug = name.slice(0, -8);
      const startMs = slugStart(slug);
      if (startMs >= FROM_MS && startMs < TO_MS && (!SLUG_ALLOWLIST || SLUG_ALLOWLIST.has(slug)) && !trades.has(slug))
        trades.set(slug, path.join(dir, name));
    }
  }
  const expected = [...(SLUG_ALLOWLIST || l2.keys())]
    .filter((slug) => slugStart(slug) >= FROM_MS && slugStart(slug) < TO_MS)
    .sort((a, b) => slugStart(a) - slugStart(b));
  discoveryCoverage.expected = expected.length;
  const rows = [];
  for (const slug of expected) {
    const missing = [];
    if (!l2.has(slug)) missing.push("executable-l2");
    if (CONFIRM_L2_DIRS.length && !confirmL2.has(slug)) discoveryCoverage.confirmationMissing.push(slug);
    if (!v2.has(slug)) missing.push("rtds-control");
    if ((process.env.MAKER_FILL_SOURCE === "trades" || STRICT_CAUSAL_FILLS) && !trades.has(slug)) missing.push("public-trades");
    if (missing.length) {
      discoveryCoverage.missing.push({ slug, missing });
      continue;
    }
    rows.push({ slug, l2File: l2.get(slug), l2Candidates: l2Candidates.get(slug) || [],
      confirmL2File: confirmL2.get(slug), confirmL2Candidates: confirmL2Candidates.get(slug) || [],
      v2File: v2.get(slug), tradeFile: trades.get(slug) });
  }
  discoveryCoverage.complete = rows.length;
  return rows;
}

const exactFlowFiles = new Set(fs.existsSync(EXACT_FLOW_DIR)
  ? fs.readdirSync(EXACT_FLOW_DIR).filter((name) => name.endsWith(".json.gz")) : []);
const exactTransactionCache = new Map();
function exactTransaction(hash) {
  const txHash = String(hash || "").toLowerCase();
  if (!/^0x[0-9a-f]{64}$/.test(txHash) || !exactFlowFiles.has(`${txHash}.json.gz`)) return null;
  if (!exactTransactionCache.has(txHash)) {
    try { exactTransactionCache.set(txHash, readGzip(path.join(EXACT_FLOW_DIR, `${txHash}.json.gz`))); }
    catch { exactTransactionCache.set(txHash, null); }
  }
  const row = exactTransactionCache.get(txHash);
  return row?.status === "decoded" ? row : null;
}

function normalize(meta) {
  currentNormalizationSlug = meta.slug;
  const densestAvailable = (primaryFile, candidates) => {
    if (!primaryFile) return null;
    let selected = readGzip(primaryFile), selectedTicks = selected.ticks?.length || 0;
    // Archive directories intentionally remain precedence ordered. Only fall
    // through when the preferred copy is itself unusably sparse.
    if (selectedTicks <= 20) for (const candidate of candidates || []) {
      if (candidate === primaryFile) continue;
      const alternative = readGzip(candidate), ticks = alternative.ticks?.length || 0;
      if (ticks > selectedTicks) { selected = alternative; selectedTicks = ticks; }
    }
    return selected;
  };
  const raw = densestAvailable(meta.l2File, meta.l2Candidates), rtds = readGzip(meta.v2File);
  const confirmRaw = densestAvailable(meta.confirmL2File, meta.confirmL2Candidates);
  const manifestMarket = SLUG_METADATA.get(meta.slug) || null;
  const openBinance = Number(rtds.openBinance);
  const openChainlink = Number(rtds.openChainlink);
  const rawWinner = raw.winner || manifestMarket?.winner;
  const winner = /^up$/i.test(rawWinner) ? "Up" : /^down$/i.test(rawWinner) ? "Down" : null;
  const upToken = String(raw.upToken || confirmRaw?.upToken || manifestMarket?.upToken || "");
  const downToken = String(raw.downToken || confirmRaw?.downToken || manifestMarket?.downToken || "");
  if (!(openChainlink > 0)) return rejectNormalization("invalid-rtds-open");
  if (!winner) return rejectNormalization("missing-winner");
  if (!INCLUDE_SPARSE_WINDOWS && (raw.sparseWindow === true || raw.isStale === true)) return rejectNormalization("v4-quality-flag");
  if (STRICT_CAUSAL_FILLS && (!upToken || !downToken)) return rejectNormalization("missing-token-ids");
  ...
  const trades = meta.tradeFile ? (readGzip(meta.tradeFile).trades || []).map((trade) => {
    const interval = publicTradeInterval(trade);
    return { ...trade, ms: Number(trade.ms), price: Number(trade.price), size: Number(trade.size),
      intervalStartMs: interval?.startMs, intervalEndMs: interval?.endMs,
      transactionHash: String(trade.transactionHash || "").toLowerCase() };
  }).filter((trade) => Number.isFinite(trade.ms) && Number.isFinite(trade.intervalStartMs)
    && (trade.side === "SELL" || trade.side === "PAIR_BUY") && trade.size > 0).sort((a, b) => a.ms - b.ms) : [];
  const decodedTransactions = new Set(), strictFlows = [];
  if (STRICT_CAUSAL_FILLS) {
    for (const trade of trades) {
      if (decodedTransactions.has(trade.transactionHash)) continue;
      const decoded = exactTransaction(trade.transactionHash);
      if (!decoded) continue;
      decodedTransactions.add(trade.transactionHash);
      for (const makerLeg of decoded.makerLegs || []) {
        const flow = economicBuyFlow(makerLeg, { upToken, downToken });
        if (!flow || !(flow.size > 0)) continue;
        strictFlows.push({ ...flow, intervalStartMs: trade.intervalStartMs, intervalEndMs: trade.intervalEndMs,
          ms: trade.intervalEndMs, transactionHash: trade.transactionHash });
      }
    }
    strictFlows.sort((a, b) => a.intervalEndMs - b.intervalEndMs || a.price - b.price || a.transactionHash.localeCompare(b.transactionHash));
  }
  ...
  const confirmTicks = confirmRaw
    ? (confirmRaw.ticks || []).map(normalizeBookTick).filter(Boolean).sort((a, b) => a.ms - b.ms)
    : [];
  return ticks.length > 20
    ? { slug: meta.slug, startMs: slugStart(meta.slug), openBinance, openChainlink, winner, upToken, downToken,
      ticks, confirmTicks, trades, strictFlows, decodedTransactions }
...
function replayWindow(feed, p) {
  ...
  let firstMakerMarketMomentum = null;
  ...
  let tradeCursor = 0, strictFlowCursor = 0, strictFlowEvents = 0;
  ...
  let residualValueCancelTriggers = 0, residualValueCancelRequests = 0;
  let lastSpotSecond = null, lastSpotPrice = null;
  const spotReturns = [];
  const releaseTrace = { Up: [], Down: [] };
  const marketMidTrace = [];
  let residualMomentumGateRejected = 0;
  let residualPersistenceGateRejected = 0;
  let residualValueSide = null, residualValueSinceMs = null;
  ...
  const residualFair = (tick, t) => {
    ...
    const blendedGapPct = aggregateSpotGap(clGapPct, bzGapPct, p.residualGapAggregation, chainlinkWeight);
    ...
    const fairModel = p.residualFairProbabilityModel;
    let fairUp;
    if (["median-pool-lower", "median-pool", "median-pool-upper"].includes(fairModel)) {
      const scale = sigmaPctPerSqrtSecond * Math.sqrt(remainingS);
      fairUp = poolFairProbability({
        harmonic: normalCdf(aggregateSpotGap(clGapPct, bzGapPct, "harmonic") / scale),
        geometric: normalCdf(aggregateSpotGap(clGapPct, bzGapPct, "geometric") / scale),
        arithmetic: normalCdf(aggregateSpotGap(clGapPct, bzGapPct, "arithmetic") / scale),
        market: marketMid,
      }, fairModel);
    } else {
      const marketWeight = clamp(Number(p.residualMarketWeight), 0, 1);
      fairUp = logistic((1 - marketWeight) * logit(brownianUp) + marketWeight * logit(marketMid));
    }
  ...
  const apply = (side, shares, price, taker, exactFee = null, makerOrder = null, fillMs = null) => {
    ...
    if (taker) { takerShares += shares; takerBuyShares += shares; takerFillEvents++; }
      ...
      if (firstMakerFillT == null && makerOrder) {
        ...
        firstMakerMarketMomentum = makerOrder.decisionMarketMomentum;
      }
  ...
  const applyMarketTradesThrough = (tickMs) => {
    while (tradeCursor < feed.trades.length && feed.trades[tradeCursor].ms <= tickMs) {
      ...
      if (STRICT_CAUSAL_FILLS) {
        const candidateOrder = pending.find((order) => trade.outcome === order.side
          && intervalFullyInsideOrder(order, { startMs: trade.intervalStartMs, endMs: trade.intervalEndMs }));
        if (candidateOrder && !feed.decodedTransactions.has(trade.transactionHash)) {
          const key = `${feed.slug}:${trade.transactionHash}`;
          if (!strictCandidateTransactions.has(key)) strictCandidateTransactions.set(key, {
            slug: feed.slug,
            transactionHash: trade.transactionHash,
            intervalStartMs: trade.intervalStartMs,
            intervalEndMs: trade.intervalEndMs,
            upToken: feed.upToken,
            downToken: feed.downToken,
          });
        }
        continue;
      }
    ...
    while (STRICT_CAUSAL_FILLS && strictFlowCursor < feed.strictFlows.length
      && feed.strictFlows[strictFlowCursor].intervalEndMs <= tickMs) {
      const flow = feed.strictFlows[strictFlowCursor++];
      const matching = pending.filter((order) => flow.outcome === order.side
        && exactPriceMatches(flow.price, order.price)
        && intervalFullyInsideOrder(order, { startMs: flow.intervalStartMs, endMs: flow.intervalEndMs }));
      if (matching.length) {
        strictFlowEvents++;
        for (const { order, shares } of allocateConservedMakerFills(matching, flow.size, p.makerCredit))
          apply(order.side, shares, order.price, false, null, order, flow.intervalEndMs);
        for (let index = pending.length - 1; index >= 0; index--)
          if (pending[index].shares <= 1e-9) pending.splice(index, 1);
      }
    }
  ...
  for (const tick of feed.ticks) {
    ...
    if (p.signalMode === "residual") {
      ...
      if (currentResidualFair) {
        marketMidTrace.push({ ms: tick.ms, up: currentResidualFair.marketMid });
        const momentumTraceHorizonMs = Math.max(30, Number(p.residualMarketMomentumLookbackS) + 5) * 1000;
        while (marketMidTrace.length && tick.ms - marketMidTrace[0].ms > momentumTraceHorizonMs) marketMidTrace.shift();
    ...
    if (p.signalMode === "residual") {
      ...
      const momentumLookbackS = Math.max(0, Number(p.residualMarketMomentumLookbackS) || 0);
      const pastMarket = momentumLookbackS > 0
        ? [...marketMidTrace].reverse().find((row) => row.ms <= tick.ms - momentumLookbackS * 1000) : null;
      const pastMarketPredicted = pastMarket
        ? (predictedSide === "Up" ? pastMarket.up : 1 - pastMarket.up) : null;
      const marketMomentum = Number.isFinite(pastMarketPredicted) ? marketPredicted - pastMarketPredicted : null;
      const marketMomentumPass = momentumLookbackS <= 0 || Number.isFinite(marketMomentum)
        && marketMomentum + 1e-12 >= Number(p.residualMinMarketMomentum);
      const directionalValueCandidate = directionalEntryOpen && predictedCurrentlySupported && marketConsensus
        && predictedEdge + 1e-12 >= minEdge;
      const otherwiseDirectional = directionalValueCandidate
        && Math.max(0, target - orientedImbalance) + 1e-9 >= p.minOrderShares;
      if (directionalValueCandidate) {
        if (residualValueSide !== predictedSide) {
          residualValueSide = predictedSide;
          residualValueSinceMs = tick.ms;
        }
      } else {
        residualValueSide = null;
        residualValueSinceMs = null;
      }
      const persistenceMs = Math.max(0, Number(p.residualValuePersistenceMs) || 0);
      const persistencePass = !otherwiseDirectional || persistenceMs <= 0
        || tick.ms - residualValueSinceMs + 1e-9 >= persistenceMs;
      if (otherwiseDirectional && !persistencePass) residualPersistenceGateRejected++;
      if (otherwiseDirectional && !marketMomentumPass) residualMomentumGateRejected++;
      const valueNeed = otherwiseDirectional && marketMomentumPass && persistencePass
      ...
      if (p.residualCancelInvalidOrders === true || p.residualCancelRepricedOrders === true) {
        ...
        const baselineOrderValid = (order) => order.side === predictedSide
          ? predictedCurrentlySupported : order.side === otherSide && otherStillIndependent;
        // A directional quote can remain on the book after its modeled edge
        // disappears even though spot still points the same way.  The optional
        // value-validity invariant closes that adverse-selection gap without a
        // new fitted threshold: it reuses the exact entry predicates.
        const valueValidityPass = directionalValueCandidate && marketMomentumPass && persistencePass;
        const newlyInvalidForValue = (order) => p.residualCancelOnValueLoss === true
          && order.side === predictedSide && order.decisionRole === "directional"
          && baselineOrderValid(order) && !valueValidityPass;
        const existingOrderValid = (order) => baselineOrderValid(order) && !newlyInvalidForValue(order);
        const cancellationReason = (order) => {
          if (p.residualCancelInvalidOrders === true && !existingOrderValid(order)) {
            return newlyInvalidForValue(order) ? "value-loss" : "invalid";
          }
          ...
          if (order.decisionRole === "balance-locked" && Number(p.residualLockedPairImproveTicks) > 0) return null;
        ...
        let requested = 0, valueRequested = 0;
        for (const order of pending) {
          ...
          if (cancelEffectiveMs + 1e-9 < order.expiresMs) {
            order.expiresMs = cancelEffectiveMs;
            requested++;
            if (reason === "value-loss") valueRequested++;
          }
        }
        for (const order of arriving) {
          const reason = cancellationReason(order);
          if (reason && order.cancelOnArrival !== true) {
            order.cancelOnArrival = true;
            requested++;
            if (reason === "value-loss") valueRequested++;
          }
        ...
        if (valueRequested > 0) {
          residualValueCancelTriggers++;
          residualValueCancelRequests += valueRequested;
        }
      }
    ...
    for (const side of ["Up", "Down"]) {
      ...
      arriving.push({ side, price: limit, shares, arrivalMs: tick.ms + p.effectiveMakerLatencyMs,
        ...
        decisionMarketMomentum: p.signalMode === "residual" ? residualPlan[side]?.marketMomentum ?? null : null,
...
function summarize(windows) {
  ...
  const evaluated = windows.filter((row) => row.unavailable !== true);
  ...
  // Candidate-discovery passes are provisional by definition. Defer the
  // expensive bootstrap until exact-flow discovery has converged, then run it
  // once on the accepted report.
  const bootstrapSamples = STRICT_CAUSAL_FILLS && strictCandidateTransactions.size > 0
    ? 0 : Number(process.env.MAKER_BOOTSTRAP_SAMPLES || 2000);
  return { windows: windows.length, evaluatedWindows: evaluated.length,
    unavailableWindows: windows.filter((row) => row.unavailable === true).length,
    activeWindows: windows.filter((row) => row.makerShares + row.takerShares > 0).length,
    ...
    residualValueCancelTriggers: sum("residualValueCancelTriggers"), residualValueCancelRequests: sum("residualValueCancelRequests"),
    residualMomentumGateRejected: sum("residualMomentumGateRejected"),
    residualPersistenceGateRejected: sum("residualPersistenceGateRejected"),
    strictFlowEvents: sum("strictFlowEvents"),
    ...
    bootstrapWindowLower95: bootstrapLower(evaluated.map((row) => row.pnl), bootstrapSamples, 0x3048d653),
...
const base = {
  ...
  residualGapAggregation: ["conservative", "harmonic", "geometric", "arithmetic",
    "lower-ensemble", "power-ensemble", "upper-ensemble"].includes(process.env.MAKER_RESIDUAL_GAP_AGGREGATION)
    ? process.env.MAKER_RESIDUAL_GAP_AGGREGATION : "weighted",
  residualFairProbabilityModel: ["median-pool-lower", "median-pool", "median-pool-upper"]
    .includes(process.env.MAKER_RESIDUAL_FAIR_PROBABILITY_MODEL)
    ? process.env.MAKER_RESIDUAL_FAIR_PROBABILITY_MODEL : "legacy",
  ...
  residualMarketMomentumLookbackS: Number(process.env.MAKER_RESIDUAL_MARKET_MOMENTUM_LOOKBACK_S || 0),
  residualMinMarketMomentum: process.env.MAKER_RESIDUAL_MIN_MARKET_MOMENTUM == null
    ? -Infinity : Number(process.env.MAKER_RESIDUAL_MIN_MARKET_MOMENTUM),
  residualValuePersistenceMs: Number(process.env.MAKER_RESIDUAL_VALUE_PERSISTENCE_MS || 0),
  residualCancelOnValueLoss: process.env.MAKER_RESIDUAL_CANCEL_ON_VALUE_LOSS === "1",
...
if (REQUIRE_FULL_COVERAGE && discoveryCoverage.missing.length && !INCLUDE_UNAVAILABLE_WINDOWS) {
  throw new Error(`all-window prerequisite coverage failed: ${JSON.stringify(discoveryCoverage)}`);
}
...
for (const meta of metas) {
  try {
    ...
    normalizationFailureRows.set(meta.slug, `exception:${String(error?.message || error).slice(0, 180)}`);
  }
}
const unavailableBySlug = new Map(discoveryCoverage.missing.map((row) => [row.slug, `missing:${row.missing.join(",")}`]));
for (const [slug, reason] of normalizationFailureRows) unavailableBySlug.set(slug, `normalization:${reason}`);
if (INCLUDE_UNAVAILABLE_WINDOWS) {
  for (const rows of windowsByKey.values()) {
    const present = new Set(rows.map((row) => row.slug));
    for (const [slug, unavailableReason] of unavailableBySlug) if (!present.has(slug)) rows.push({
      slug,
...
const output = {
  ...
  methodology: STRICT_CAUSAL_FILLS
    ? "strict causal passive GTC replay: public Data API timestamps are treated as one-second intervals and a fill is eligible only when the entire interval is inside the simulated order's live interval; exact CLOB V2 matchOrders calldata supplies each resting maker leg's token, side, limit price, and matched amount; exact economic-price flow consumes visible queue ahead before one shared FIFO volume-conserved maker-credit budget; V2/V4 books remain alternative executable reconstructions and confirm each other; RTDS TWAP-60 controls only; post-only arrival checks; zero maker fee/rebate dependency; settlement outcome used only for PnL"
    : "causal passive GTC replay: public CLOB plus official RTDS TWAP-60 controls only; optional independent v2/v4 orderbook confirmation is fail-closed for opening-cycle decisions and uses the more conservative bid; confirmed normalized market-wide taker prints at the exact one-cent maker level consume visible queue ahead before a shared, FIFO, volume-conserved maker-credit budget; post-only arrival checks; crypto taker fee 0.07*p*(1-p)*shares rounded to five decimals per consumed price level; maker rebates are an explicit policy sensitivity equal to makerRebateRate times each maker fill's same-curve fee equivalent and are reported separately from trading PnL; settlement outcome used only for PnL",
  range: { from: new Date(FROM_MS).toISOString(), to: new Date(TO_MS).toISOString(), discovered: discoveryCoverage.expected, sourceComplete: metas.length,
    loaded, failed, represented: configs.length ? windowsByKey.values().next().value.length : 0, unavailable: unavailableBySlug.size,
    normalizationFailures },
  coverage: discoveryCoverage,
  strictFillAudit: STRICT_CAUSAL_FILLS ? {
    enabled: true,
    exactFlowDir: EXACT_FLOW_DIR,
    exactTransactionsAvailable: exactFlowFiles.size,
    unresolvedCandidateTransactions: strictCandidateTransactions.size,
    candidateOutput: STRICT_CANDIDATE_OUTPUT,
    intervalRule: "intervalStartMs >= effectiveArrivalMs && intervalEndMs <= expiresMs",
  } : { enabled: false },
  provenance: {
    l2Dirs: L2_DIRS,
    confirmationL2Dirs: CONFIRM_L2_DIRS,
    v2Dirs: V2_DIRS,
    tradeDirs: TRADE_DIRS,
    exactFlowDir: STRICT_CAUSAL_FILLS ? EXACT_FLOW_DIR : null,
    slugAllowlist: SLUG_ALLOWLIST_FILE ? {
      path: path.resolve(SLUG_ALLOWLIST_FILE),
      sha256: crypto.createHash("sha256").update(fs.readFileSync(path.resolve(SLUG_ALLOWLIST_FILE))).digest("hex"),
      slugs: SLUG_ALLOWLIST.size,
    } : null,
  },
...
if (STRICT_CAUSAL_FILLS) {
  const transactions = [...strictCandidateTransactions.values()].sort((a, b) => a.slug.localeCompare(b.slug)
    || a.intervalStartMs - b.intervalStartMs || a.transactionHash.localeCompare(b.transactionHash));
  fs.mkdirSync(path.dirname(STRICT_CANDIDATE_OUTPUT), { recursive: true });
  fs.writeFileSync(STRICT_CANDIDATE_OUTPUT, JSON.stringify({
    schema: 1,
...
if (REQUIRE_FULL_COVERAGE && (configs.length && windowsByKey.values().next().value.length !== discoveryCoverage.expected)) process.exitCode = 2;
else if (STRICT_CAUSAL_FILLS && strictCandidateTransactions.size > 0) process.exitCode = 3;
